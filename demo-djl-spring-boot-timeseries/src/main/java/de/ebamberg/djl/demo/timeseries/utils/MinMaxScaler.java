...
public class MinMaxScaler implements AutoCloseable {
    ...
    /**
     * Sets the minimum and maximum to the specific range for later scaling.
     *
     * <p>The min-max arrays are stored by reference. Therefore the ndarrays are still attached to
     * the NDManager. you have to call {@code detach()} on the scaler to detach this from the
     * NDManager even if the MinMaxScaler has been detached before.
     *
     * @param minimum to use for later scaling
     * @param maximum to use for later scaling
     * @return the fitted MinMaxScaler
     */
    public MinMaxScaler fitToRange(NDArray minimum, NDArray maximum) {
        fittedMin = minimum;
        fittedMax = maximum;
        fittedRange = fittedMax.sub(fittedMin);
        return this;
